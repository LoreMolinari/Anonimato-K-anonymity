...
import sqlite3
import pandas as pd
from utils.data_loader_bank import load_dataB, default_data_configB, display_tableB
...
config = {'data': default_data_configB, 'samarati': True}
data_original = load_dataB(config)['table']
...
data_anonymizedM = pd.read_csv('results/mondrian_bank.csv')
data_anonymizedS = pd.read_csv('results/samarati_bank.csv')
...
query_anonymizedM = f"SELECT * FROM anonymizedM WHERE \"{valid_column_anonymizedM}\" = \"{data_anonymizedM[valid_column_anonymizedM].iloc[433]}\""
