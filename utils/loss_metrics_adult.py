...
def categorical_loss_metric_adult(qi_columns, leaves_num, hierarchies, sup):
...
def compute_numerical_loss_metric(column):
    ...
    return loss / (max_range * len(column))  # media


def numerical_loss_metric_adult(qi_columns):
    loss_metric = 0
    for attr in qi_columns:
        col = qi_columns[attr].tolist()
